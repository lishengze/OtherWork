...
{
	/// <summary>
	/// 绩效考核_股票每日交易汇总大表_格式不符
	/// </summary>
	public partial class 绩效考核_股票每日交易汇总大表_格式不符
	{
		private readonly Maticsoft.DAL.绩效考核_股票每日交易汇总大表_格式不符 dal=new Maticsoft.DAL.绩效考核_股票每日交易汇总大表_格式不符();
		public 绩效考核_股票每日交易汇总大表_格式不符()
		{}
		#region  Method
		/// <summary>
		/// 是否存在该记录
        ...
        public DataTable GetOutPutTable(string strWhere)
		{
            ...
            string sql = "select 股票代码 as 代码, 股票名称 as 名称, 基金经理, 持股数量, 持股成本,市场现价, 投资成本,今日市值,浮盈浮亏,投资成本占比,市值占比,浮盈浮亏率,本年净值贡献='',当日盈亏  from 绩效考核_股票每日交易汇总大表_格式不符 where " + strWhere;
        ...
        public DataTable Get_统计信息(string 产品名称, string 基金经理, string startTime, string endTime)
        {
            ...
            if(基金经理=="全部")
                sql = string.Format("SELECT  股票代码,股票名称, MAX(投资成本) as 最大投资金额, SUM(当日盈亏) as 实现盈亏 FROM 绩效考核_股票每日交易汇总大表_格式不符 " +
        ...
        {
            double 期间买卖总盈亏 = 0;
            string sql = string.Format("SELECT  SUM(当日盈亏)  FROM 绩效考核_股票每日交易汇总大表_格式不符 where 产品名称= '{0}' and 基金经理= '{1}' and 时间 between '{2}' and '{3}' ", 产品名称,基金经理, startTime, endTime);
        ...
        {
            double 期间总浮盈浮亏 = 0;
            string sql = string.Format("SELECT  SUM(浮盈浮亏)  FROM 绩效考核_股票每日交易汇总大表_格式不符 where 产品名称= '{0}' and 基金经理= '{1}' and 时间 between '{2}' and '{3}' ", 产品名称, 基金经理, startTime, endTime);
            object obj = DbHelperSQL.GetSingle(sql);
            if (obj != null)
            {
        ...
        { 
            ...
            strSql.Append("select 时间,产品名称,股票代码,股票名称,基金经理,持股数量,持股成本,市场现价,投资成本,今日市值,浮盈浮亏,投资成本占比,市值占比,浮盈浮亏率,当日盈亏 FROM 绩效考核_股票每日交易汇总大表_格式不符 ");
            if (strWhere.Trim() != "")
            {
        ...
        { 
            string sql = string.Format("select t1.*,t2.本年净值贡献 from (select *  FROM 绩效考核_股票每日交易汇总大表_格式不符  where {0}) t1  left join 绩效考核_基金经理净值贡献表 t2  on t1.基金经理= t2.基金经理 and t1.产品名称= t2.基金产品 and t1.时间= t2.时间  order by 股票名称, 投资成本 desc ", strWhere);
        ...
        { 
            ...
            string sql1 = "select t1.*,t2.今日买入股,t2.今日卖出股,t2.买入清算金额,t2.卖出清算金额 from " +
                        "(select {0} from 绩效考核_股票每日交易汇总大表_格式不符 where 时间 between '{1}' and  '{2}' and 产品名称 ='{3}') t1 left join  绩效考核_股票每日交易汇总小表 t2 " +
