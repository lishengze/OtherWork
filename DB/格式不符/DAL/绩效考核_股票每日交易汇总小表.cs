...
{
    /// <summary>
    /// 数据访问类:绩效考核_股票每日交易汇总小表_格式不符
    /// </summary>
    public partial class 绩效考核_股票每日交易汇总小表_格式不符
    {
        public 绩效考核_股票每日交易汇总小表_格式不符()
        { }
        #region  Method
        ...
        public bool Exists(long 记录标识)
        {
            ...
            strSql.Append("select count(1) from 绩效考核_股票每日交易汇总小表_格式不符");
        ...
        {
            long result = 0; 
            string sql = string.Format("select 记录标识 from 绩效考核_股票每日交易汇总小表_格式不符 where 股票代码='{0}' and 基金经理='{1}' and 产品名称='{2}' and 时间 = '{3}'", 股票代码, 基金经理, 产品名称, 开始时间);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into 绩效考核_股票每日交易汇总小表_格式不符(");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update 绩效考核_股票每日交易汇总小表_格式不符 set ");
        ...
        public bool Delete(long 记录标识)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from 绩效考核_股票每日交易汇总小表_格式不符 ");
            strSql.Append(" where 记录标识=@记录标识 ");
            SqlParameter[] parameters = {
					new SqlParameter("@记录标识", SqlDbType.BigInt,8)};
            parameters[0].Value = 记录标识;
        ...
        public Maticsoft.Model.绩效考核_股票每日交易汇总小表 GetModel(long 记录标识)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 记录标识,股票代码,基金经理,产品名称,股票名称,时间,今日买入股,买入均价,买入金额,今日卖出股,卖出均价,卖出金额,买入手续费,买入过户费,买入印花税,卖出手续费,卖出过户费,卖出印花税,买入清算金额,卖出清算金额,是否为止损指令 from 绩效考核_股票每日交易汇总小表_格式不符 ");
            strSql.Append(" where 记录标识=@记录标识 ");
            SqlParameter[] parameters = {
					new SqlParameter("@记录标识", SqlDbType.BigInt,8)};
            parameters[0].Value = 记录标识;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select 记录标识,产品名称,基金经理,股票代码,股票名称,时间,今日买入股,买入均价,买入金额,今日卖出股,卖出均价,卖出金额,买入手续费,买入过户费,买入印花税,卖出手续费,卖出过户费,卖出印花税,买入清算金额,卖出清算金额,是否为止损指令 from 绩效考核_股票每日交易汇总小表_格式不符 ");
            if (strWhere.Trim() != "")
            {
