...
namespace Maticsoft.DAL
{
	...
	public partial class 绩效考核_基金产品每日统计
	{
		...
		public bool Exists(long 记录标识)
		{
			...
			strSql.Append("select count(1) from 绩效考核_基金产品每日统计");
        ...
        {
            long result = 0;
            string sql = string.Format("select 记录标识 from 绩效考核_基金产品每日统计 where 产品名称='{0}' and 时间 = '{1}'", 产品名称, 开始时间);
		...
		{	  
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into 绩效考核_基金产品每日统计(");
		...
		{ 
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from 绩效考核_基金产品每日统计 ");
			strSql.Append(" where 记录标识=@记录标识 ");
			SqlParameter[] parameters = {
					new SqlParameter("@记录标识", SqlDbType.BigInt,8)};
			parameters[0].Value = 记录标识;
		...
		public Maticsoft.Model.绩效考核_基金产品每日统计 GetModel(long 记录标识)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 * from 绩效考核_基金产品每日统计 ");
			strSql.Append(" where 记录标识=@记录标识 ");
			SqlParameter[] parameters = {
					new SqlParameter("@记录标识", SqlDbType.BigInt,8)};
			parameters[0].Value = 记录标识;
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select *  FROM 绩效考核_基金产品每日统计 ");
			if(strWhere.Trim()!="")
			{
