...
namespace Maticsoft.DAL
{
    ...
    public partial class 绩效考核_股票每日交易汇总大表
    {
        ...
        public bool Exists(long 记录标识)
        {
            ...
            strSql.Append("select count(1) from 绩效考核_股票每日交易汇总大表");
        ...
        {
            long result = 0;
            string  sql = string.Format("select 记录标识 from 绩效考核_股票每日交易汇总大表 where 股票代码='{0}' and 产品名称='{1}' and 时间 = '{2}' and 基金经理='{3}'", 股票代码, 产品名称, 开始时间, 基金经理);
        ...
        { 
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into 绩效考核_股票每日交易汇总大表(");
            strSql.Append("记录标识,产品名称,基金经理,股票代码,股票名称,持股数量,持股成本,市场现价,投资成本,今日市值,浮盈浮亏,投资成本占比,市值占比,浮盈浮亏率,当日盈亏,时间,买卖累计盈亏,今日均价,是否修改过持股数量和持股成本)");
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@是否修改过持股数量和持股成本", SqlDbType.Bit,1)
            ...
            parameters[18].Value = model.是否修改过持股数量和持股成本;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update 绩效考核_股票每日交易汇总大表 set ");
            ...
            strSql.Append("是否修改过持股数量和持股成本=@是否修改过持股数量和持股成本");
            ...
            parameters[17].Value = model.是否修改过持股数量和持股成本;
            parameters[18].Value = model.记录标识;
        ...
        public bool Delete(long 记录标识)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from 绩效考核_股票每日交易汇总大表 ");
            strSql.Append(" where 记录标识=@记录标识 ");
            SqlParameter[] parameters = {
					new SqlParameter("@记录标识", SqlDbType.BigInt,8)};
            parameters[0].Value = 记录标识;
        ...
        public Maticsoft.Model.绩效考核_股票每日交易汇总大表 GetModel(long 记录标识)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 记录标识,产品名称,基金经理,股票代码,股票名称,持股数量,持股成本,市场现价,投资成本,今日市值,浮盈浮亏,投资成本占比,市值占比,浮盈浮亏率,当日盈亏,时间,买卖累计盈亏,今日均价,是否修改过持股数量和持股成本  from 绩效考核_股票每日交易汇总大表 ");
            strSql.Append(" where 记录标识=@记录标识 ");
            SqlParameter[] parameters = {
					new SqlParameter("@记录标识", SqlDbType.BigInt,8)};
            parameters[0].Value = 记录标识;
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["是否修改过持股数量和持股成本"] != null && ds.Tables[0].Rows[0]["是否修改过持股数量和持股成本"].ToString() != "")
                {
                    if ((ds.Tables[0].Rows[0]["是否修改过持股数量和持股成本"].ToString() == "1") || (ds.Tables[0].Rows[0]["是否修改过持股数量和持股成本"].ToString().ToLower() == "true"))
                    {
        ...
        public DataSet GetList(string strWhere)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select 记录标识,产品名称,股票代码,股票名称,基金经理,持股数量,持股成本,市场现价,投资成本,今日市值,浮盈浮亏,投资成本占比,str(市值占比,16,14) as 市值占比,浮盈浮亏率,当日盈亏,时间,买卖累计盈亏,今日均价,是否修改过持股数量和持股成本 FROM 绩效考核_股票每日交易汇总大表 ");
            if (strWhere.Trim() != "")
            {
