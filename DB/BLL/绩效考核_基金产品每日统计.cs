...
namespace Maticsoft.BLL
{
	...
	{
		private readonly Maticsoft.DAL.绩效考核_基金产品每日统计 dal=new Maticsoft.DAL.绩效考核_基金产品每日统计();
		...
		/// 获取两个时间段内的最大净值
		/// </summary>
        public Dictionary<string,double> Get_今年最大净值(string beginTime,string endTime)
		{
            Dictionary<string, double> DIC = new Dictionary<string, double>();
            string sql = string.Format(" select 产品名称,max(单位净值)  as 今年最大净值 from 绩效考核_基金产品每日统计 where 时间 between '{0}' and  '{1}' group by 产品名称",beginTime,endTime);
            ...
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    ...
                    {
                        double 今年最大净值=0;
                        double.TryParse(row["今年最大净值"].ToString(), out 今年最大净值);
        ...
        public double Get指定时间段内第一次出现该产品的单位净值(string beginTime, string endTime,string 产品名称)
        {
            double 单位净值 = 0;
            string sql = string.Format("select top 1 单位净值 from 绩效考核_基金产品每日统计 where 时间 between '{0}' and '{1}' and 产品名称 = '{2}' order by 时间", beginTime, endTime, 产品名称);
            DataSet ds=  DbHelperSQL.Query(sql);
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    ...
                    {
                        double.TryParse(ds.Tables[0].Rows[0]["单位净值"].ToString(), out 单位净值);
                    }
                }
            }
            return 单位净值; 
        ...
        public bool UpdatePatial_期货资产(Maticsoft.Model.绩效考核_基金产品每日统计 model)
        {
            ...
            strSql.Append("update 绩效考核_基金产品每日统计 set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select 时间,产品名称,资产总额 as 总资产,单位净值,资金资产比例 FROM 绩效考核_基金产品每日统计 ");
            if (strWhere.Trim() != "")
            {
		...
		{
            string sql = string.Format("select distinct(时间) from 绩效考核_基金产品每日统计 where 时间 < '{0}' order by 时间 desc", 今日时间);
            DataSet ds =DbHelperSQL.Query(sql);
