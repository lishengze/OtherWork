...
namespace 基金管理
{
    public class ExcelEdit
    {
        ...
        public void WriteData(System.Data.DataTable data, int startRow, int startColumn)
        {
            for (int i = 0; i <= data.Rows.Count - 1; i++)
            {
                for (int j = 0; j <= data.Columns.Count - 1; j++)
                {
                    ...
                    if (data.Rows[i][j].ToString() != "0")  //added by qhc (如果值为0，则不再写入),增加了if判断
                    {
                        myExcel.Cells[startRow + i, startColumn + j] = data.Rows[i][j].ToString();
                    } 
                }
        ...
        {
            Excel.Worksheet worksheet = (Worksheet)myExcel.ActiveSheet;
            //后面的数字表示位置，位置默认
        ...
        public void setCells_TextFormat(int startRow, int startColumn, int endRow, int endColumn)
        {
            Excel.Range r1 = myExcel.get_Range(myExcel.Cells[startRow, startColumn], myExcel.Cells[endRow, endColumn]);
            r1.NumberFormatLocal = "@";
        }

        /// <summary>
        /// 设置单元格格式；--保留小数位数，并使用千位分隔符 
        /// </summary>
        /// <param name="startRow">起始行</param>
        /// <param name="startColumn">起始列</param>
        /// <param name="endRow">结束行</param>
        /// <param name="endColumn">结束列</param>
        public void setCells_NumberFormat(int startRow, int startColumn, int endRow, int endColumn, int 小数位数)
        {
            Excel.Range r1 = myExcel.get_Range(myExcel.Cells[startRow, startColumn], myExcel.Cells[endRow, endColumn]);
            if (小数位数 == 2)
                r1.NumberFormatLocal = "#,##0.00";
            else if (小数位数 == 4)
                r1.NumberFormatLocal = "#,##0.0000";
            else if (小数位数 == 0)
                r1.NumberFormatLocal = "#,###";
            else
                r1.NumberFormatLocal = "#,##0.000";
        }
        ...
        /// <param name="endRow">结束行</param>
        /// <param name="endColumn">结束列</param>
        public void setCells_PercentFormat(int startRow, int startColumn, int endRow, int endColumn)
        {
            Excel.Range r1 = myExcel.get_Range(myExcel.Cells[startRow, startColumn], myExcel.Cells[endRow, endColumn]);
            r1.NumberFormatLocal = "0.00%";
        }
        ...
        /// <summary>
        /// 合并单元格并赋值--added by qhc
        /// </summary>
        /// <param name="startRow">起始行</param>
        /// <param name="startColumn">起始列</param>
        /// <param name="endRow">结束行</param>
        /// <param name="endColumn">结束列</param>
        public void CellsUnite(int startRow, int startColumn, int endRow, int endColumn, string text)
        {
            Excel.Range range = myExcel.get_Range(myExcel.Cells[startRow, startColumn], myExcel.Cells[endRow, endColumn]);
        ...
        /// <param name="endRow">结束行</param>
        /// <param name="endColumn">结束列</param>
        public void CellsUnite(int startRow, int startColumn, int endRow, int endColumn, double number)
        { 
            Excel.Range range = myExcel.get_Range(myExcel.Cells[startRow, startColumn], myExcel.Cells[endRow, endColumn]);
        ...
        public void ColumnAutoFit(int columnNum)
        {
            ...
            //获取当前正在使用的工作表
            Excel.Worksheet worksheet = (Excel.Worksheet)myExcel.ActiveSheet;
            Excel.Range range = (Range)worksheet.Columns[strcolumnNum + ":" + strcolumnNum, System.Type.Missing];
            range.EntireColumn.AutoFit();
        ...
        public void SetRangeBackground(int startRow, int startColumn, int endRow, int endColumn, System.Drawing.Color color)
        {
            Excel.Range range = myExcel.get_Range(myExcel.Cells[startRow, startColumn], myExcel.Cells[endRow, endColumn]);
        ...
        public bool SaveAs(string fileName)
        {
            try
            {
                string Version = myExcel.Version;//获取你使用的excel 的版本号
                ...
                myWorkBook.SaveAs(fileName, FormatNum, Type.Missing, Type.Missing, Type.Missing, Type.Missing, XlSaveAsAccessMode.xlExclusive, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
                return true; 
            }
            catch (Exception ex)
            {
